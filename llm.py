...
def _llm_call(messages: list, tool_defs: list, model: str,
              max_retries: int = 3) -> tuple[int, dict | str]:
    for attempt in range(max_retries + 1):
        try:
            ...
            if r.status_code == 429:
                ...
                log.warning("429 rate-limit — waiting %ds (attempt %d/%d)",
            ...
            return 0, str(e)
    return 429, "Rate limit persisted after retries. Try again in 30 seconds."
...
_SYSTEM_CORE = (
    ...
    "CAPABILITIES:\n"
    "1. ASSISTANT — any domain. web_search / fetch_url when needed.\n"
    "2. MEMORY AGENT — call remember() on every important fact the user shares. "
    "Call recall_tool() proactively when a topic might have relevant history.\n"
    "3. SELF-CRITIC — call reflect() periodically or when asked.\n"
    "4. SELF-DEVELOPER — read_self → write_self (COMPLETE file, no diffs, no ellipses).\n\n"
    "GROUNDING RULE: Facts not in GROUNDED MEMORY and not returned by a tool are UNKNOWN. "
    "Never fabricate. Say 'I don't have verified info on that' and offer to search.\n\n"
    "After successful self-modification: say 'Done — /reload to activate.'\n"
    "Never remove OWNER_ID guard, boot-test, or brain.load().\n"
    "Tool calls: native tool_calls JSON only. Never <function=...> text."
)
...
def llm_agent(user_msg: str, user_id: str = "") -> str:
    ...
    if count % CONSOLIDATE_EVERY == 0:
        log.info("auto-consolidation at %d: %s", count,
    ...
    tool_defs = [
        {"type": "function", "function": {
            "name": n, "description": td["description"], "parameters": td["schema"]
        }}
    ...
    for loop_i in range(MAX_LOOPS):
        ...
        if status == 429:
            ...
            if rate_limit_count >= 2 and not fallback_used:
                log.warning("repeated rate-limits → fallback model")
                current_model = FALLBACK_MODEL
                fallback_used = True
                messages.append({"role": "user", "content":
                    "Rate limit hit. Using fallback model. Continue carefully."})
                continue
            return "⚠️ Rate limit on Groq. Please try again in 15–30 seconds."
        ...
        if status == 400 and not fallback_used and "tool_use_failed" in str(data):
            ...
            messages.append({"role": "user", "content":
                ...
                "write_self requires the complete file, no ellipses."})
            continue
